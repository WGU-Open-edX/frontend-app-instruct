...
import { useLearnerGrading } from '../data/apiHook';
...
const GradingLearnerContent = ({ toolType }: GradingLearnerContentProps) => {
  ...
  const { isLoading } = useLearnerGrading(courseId, emailOrUsername);
  ...
  const handleProblemChange = (event: React.MouseEvent<HTMLButtonElement>): void => {
    console.log('Problem specified for report generation', event);
  };
